		...
		for _, t := range file.Entries {
			if a, ok := t.Annotations[resourceAnnotation]; ok {
				...
				if descriptors, ok := desc.Features.Get(ResourceFeatureKind, RFResources); ok {
					...
					desc.Features.Set(ResourceFeatureKind, RFManagerField, cg.proj.CallCodeFeatureFunc(t, ServiceFeatureKind, SFKEngineService, resource.ServiceManager, resource.ServiceManager))
				}
				if checkAccess {
					desc.Features.Set(ResourceFeatureKind, RFAccessCheckerField, cg.proj.CallCodeFeatureFunc(t, ServiceFeatureKind, SFKEngineService, resource.ServiceAccessChecker, resource.ServiceAccessChecker))
				}
			}
		}
	}
