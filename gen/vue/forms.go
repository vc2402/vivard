...
const htmlFormArrayAsListTemplate = `{{define "ARRAY_AS_LIST"}}<div class="d-flex flex-column mt-2">
  <div class="d-flex flex-row justify-space-around"><h3>{{Label .}}</h3>{{if not (Readonly .)}}<v-btn if="!disabled" text color="primary" @click="add{{FieldName . "U"}}"><v-icon>add</v-icon> {{Label .}}</v-btn>{{end}}</div>
    ...
    <div class="d-flex flex-column" v-for="(it, idx) in value.{{FieldName .}}"  :key="idx">
      <div class="d-flex flex-row align-center justify-space-between mt-3">
        ...
        <div class="flex-grow-0 flex-shrink-1 ml-2">
          <v-icon if="!disabled" color="error" @click="remove{{FieldName . "U"}}(idx)" title="delete">mdi-close-circle</v-icon>
        </div>
      </div>
